...
export class CategoriesService {
  ...
  async create (userId: number, { name }: CreateCategoryDto) {
    const category = await this.prismaService.client.categoryModel.create({ data: { name, userModelId: userId } });
    this.logger.info(`[CategoriesService] category created: ${JSON.stringify(category)}`);
    return category;
  };

  async getAll (userId: number) {
    const categories = await this.prismaService.client.categoryModel.findMany({ where: { userModelId: userId } });
